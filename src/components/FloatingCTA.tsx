...
const FloatingCTA = () => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-30 animate-fade-in safe-area-inset-bottom">
      <div className="bg-background/95 backdrop-blur-sm border border-border/50 rounded-full shadow-elegant px-4 py-2 flex items-center gap-2 mx-4 max-w-xs">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          />
          <span className="text-xs font-medium text-foreground truncate">Curately</span>
        </div>
        
        <Link to="/auth">
          <Button 
            size="sm" 
            className="bg-gradient-primary hover:opacity-90 transition-opacity text-xs px-3 py-1 h-7 flex-shrink-0"
          >
            Try free
          </Button>
        </Link>
